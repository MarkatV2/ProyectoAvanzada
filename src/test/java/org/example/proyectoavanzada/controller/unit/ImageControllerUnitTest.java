...
class ImageControllerUnitTest {
    ...
    void setUp() {
        ...
        //uploadRequest = new ImageUploadRequest("https://example.com/image.jpg");
    }

//    @Nested
//    @DisplayName("GET /api/v1/images/{imageId}")
//    class GetImageByIdTest {
        ...
        void shouldReturnNotFoundWhenImageDoesNotExist() throws Exception {
            // Arrange
            when(imageService.getImageById(imageId)).thenThrow(new IllegalArgumentException("Imagen no encontrada"));
...
//    class DeleteImageTest {
        ...
        @Test
        @DisplayName("❌ Debe retornar 403 si el usuario no tiene permiso")
        void shouldReturnForbiddenWhenNotAuthorized() throws Exception {
            // Arrange
            doThrow(new SecurityException("No tiene permiso")).when(imageService).deleteImage(imageId);

            // Act & Assert
            mockMvc.perform(delete("/api/v1/images/{id}", imageId))
                    .andExpect(status().isForbidden())
                    .andExpect(content().string(containsString("No tiene permiso")));
        }
        ...
        void shouldReturnNotFoundWhenImageNotFound() throws Exception {
            // Arrange
            doThrow(new IllegalArgumentException("Imagen no encontrada")).when(imageService).deleteImage(imageId);
